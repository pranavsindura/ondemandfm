...
const getSearchResult = (query)=>{
  return dispatch => {
    return axios.post(`https://api.dirble.com/v2/search?token=${token}`,{query: query}, header)
...
const getPopular = () => {
  return (dispatch) => {
    // return axios.get(`${proxyURL}/https://api.dirble.com/v2/stations?token=${token}`, header)
    return axios.get(`https://api.dirble.com/v2/stations/popular?token=${token}`, header)
